...
const nextConfig = {
  ...
  async rewrites() {
    ...
    const bookingService =
      process.env.BOOKING_SERVICE_URL || "http://localhost:4001";
    const orderService =
      process.env.ORDER_SERVICE_URL || "http://localhost:4002";
    const productService =
      process.env.PRODUCT_SERVICE_URL || "http://localhost:4003";
    const profileService =
      process.env.PROFILE_SERVICE_URL || "http://localhost:4004";
    const recommenderService =
      process.env.RECOMMENDER_SERVICE_URL || "http://localhost:4005";
    ...
    return [
      ...
      // Payment Service - Port 4002
      {
        ...
        destination: `${orderService}/api/payment/:path*`,
      },
